...
public class Day10_part1 {
    ...
    for (int j = 0; j < data.size(); j++) {
      for (int k = 0; k < data.get(0).size(); k++) {
        if (data.get(j).get(k) == 0) {
          HashSet<List<Integer>> found = countNeighborHills(j, k);
  ...
  private static HashSet<List<Integer>> countNeighborHills(int j, int k) {
    int currentVal = data.get(j).get(k);
    
    HashSet<List<Integer>> indices = new HashSet<List<Integer>>();
